...
st.markdown("""
    ...
    div.stButton > button { border-radius: 12px !important; font-weight: 700 !important; transition: all 0.3s; }
    </style>
...
try:
    ...
    st.error("Errore nelle chiavi dei Secrets. Controlla la configurazione.")
...
if st.session_state.user_id is None:
    ...
    if st.session_state.is_premium == "Free":
        ...
        if st.sidebar.button("ATTIVA GOLD", use_container_width=True):
            session = stripe.checkout.Session.create(payment_method_types=['card'], line_items=[{'price': ID_GOLD, 'quantity': 1}], mode='subscription', success_url="https://svuotafrigo-app.streamlit.app/", cancel_url="https://svuotafrigo-app.streamlit.app/", customer_email=st.session_state.user_email)
            st.sidebar.link_button("💳 PAGA ORA", session.url, use_container_width=True)
...
with t1:
    ...
    if st.session_state.ultima_ricetta:
        ...
        if st.session_state.user_id and st.button("💾 SALVA IN ARCHIVIO", use_container_width=True):
            try:
...
with t2:
    ...
    if st.session_state.user_id:
        with st.form("dispensa_form"):
            ...
            if st.form_submit_button("Salva in dispensa"):
                supabase.table("dispensa").insert({"user_id": st.session_state.user_id, "ingrediente": new_item}).execute()
                st.success(f"{new_item} aggiunto!")
        
        items = supabase.table("dispensa").select("*").eq("user_id", st.session_state.user_id).execute()
        for i in items.data: st.write(f"✅ {i['ingrediente']}")
    else: st.error("Accedi!")
...
with t5:
    ...
    if st.session_state.user_id:
        ...
        commento = st.text_area("Suggerimenti")
        if st.button("Invia"):
            supabase.table("feedback").insert({"user_id": st.session_state.user_id, "voto": voto, "commento": commento}).execute()
            st.success("Grazie!")
