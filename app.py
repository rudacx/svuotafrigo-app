...
if st.session_state.user_id is None:
    ...
    if st.session_state.is_premium == "Free":
        ...
        if st.sidebar.button("ATTIVA ORA", key="gold"):
            ...
            st.sidebar.link_button("💳 VAI AL PAGAMENTO", sess.url)
...
with t1:
    ...
    if st.session_state.ultima_ricetta:
        ...
        if st.session_state.user_id and st.button("💾 SALVA IN ARCHIVIO"):
            try:
                # FIX: Usiamo 'contenuto' per far combaciare con il tuo DB
                supabase.table("ricette").insert({
                    "user_id": st.session_state.user_id, 
                    "contenuto": st.session_state.ultima_ricetta
                }).execute()
                st.success("Salvata correttamente!")
...
with t2:
    ...
    else: st.warning("Esegui il login per gestire la dispensa.")
    ...
    if st.session_state.user_id:
        ...
        for r in res.data:
            with st.expander(f"Ricetta del {r['created_at'][:10]}"):
                # Usiamo 'contenuto' anche qui per leggere
...
with t5:
    ...
    if st.session_state.user_id:
        ...
        testo = st.text_area("Suggerimenti")
        if st.button("Invia"):
            supabase.table("feedback").insert({"user_id": st.session_state.user_id, "voto": voto, "commento": testo}).execute()
